...
const GUILD_ID = '1545708846553501768';

// قائمة البوتات الـ 8 مع ربط كل بوت برومه المخصص ومتغير التوكن الخاص به في ريلواي
const botsConfig = [
...
function startBot(config) {
    ...
    client.once('ready', () => {
        console.log(`[+] تم تشغيل بنجاح: ${client.user.tag} (${config.name})`);
...
function connectToVoiceChannel(client, config) {
    ...
    try {
        ...
        // نظام الحماية: إذا فصل البوت لأي سبب، يرجع يدخل رومه فوراً وبدون ما يطلع
        ...
        console.log(`[🔊] (${config.name}) قفل وثبت في الروم: ${channel.name} بنجاح 24/7`);
    } catch (error) {
...
// تشغيل البوتات الـ 8 مع بعض في نفس الوقت
