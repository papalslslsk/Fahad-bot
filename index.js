...
function startBot(botData, fileName) {
    if (!botData.token || !botData.channelId) {
        console.log(`[-] الملف ${fileName} بياناته ناقصة (Token أو Channel ID).`);
        return;
    }
...
// قراءة ملفات الـ json مباشرة من المجلد الأساسي بدون الحاجة لمجلد bots
const files = fs.readdirSync(projectDir).filter(file => file.startsWith('bot') && file.endsWith('.json'));

if (files.length === 0) {
    console.error("[-] خطأ: لم يتم العثور على أي ملفات بوتات (bot*.json)!");
    process.exit(1);
}
